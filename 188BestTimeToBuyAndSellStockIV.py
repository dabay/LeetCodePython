...
=== Comments by Dabay===
以下是摘自：http://blog.csdn.net/linhuanmars/article/details/23236995

解释最多可以进行k次交易的算法，然后最多进行两次我们只需要把k取成2即可。我们还是使用“局部最优和全局最优解法”。
...
class Solution:
    ...
    def maxProfit(self, k, prices):
        ...
        transaction = min(k, len(prices))
        helper = [0] * (transaction + 1)
        res = [0] * (transaction + 1)
        for i in xrange(1, len(prices)):
            last_profit = prices[i] - prices[i-1]
            for j in xrange(transaction, 0, -1):
                helper[j] = max(helper[j] + last_profit, res[j-1] + max(0, last_profit))
                res[j] = max(res[j], helper[j])
